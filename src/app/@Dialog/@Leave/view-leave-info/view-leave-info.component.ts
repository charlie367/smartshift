...
import { format, startOfMonth, lastDayOfMonth, addDays, addMonths } from 'date-fns';
...
export class ViewLeaveInfoComponent {
  ...
  //初始化
  ngOnInit(): void {
    this.http.getApi(`http://localhost:8080/leave/getApprovedLeave?start=${format(this.firstMonth,'yyyy-MM-dd')}&end=${format(lastDayOfMonth(this.firstMonth),'yyyy-MM-dd')}`).subscribe((res:any)=>{
