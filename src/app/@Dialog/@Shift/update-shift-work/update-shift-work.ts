...
import { addDays, endOfMonth, endOfWeek, format, isWithinInterval, lastDayOfMonth, parseISO, startOfWeek, subDays} from 'date-fns';
...
export class UpdateShiftWork {
  ...
  //初始化
  ngOnInit(): void {
    this.http.getApi(`http://localhost:8080/PreSchedule/prettyScheduleNotLeave?start=${format(this.employeeInfo.firstDayOfMonth,'yyyy-MM-dd')}&end=${format(endOfMonth(this.employeeInfo.firstDayOfMonth),'yyyy-MM-dd')}`).subscribe((res:any)=>{
  ...
  cancelShift(index:number){
    ...
    this.http.postApi(`http://localhost:8080/PreSchedule/deleteSchedule`,data).subscribe((res:any)=>{
      if(res.code == 200){
  ...
  updateItem(index:number){
    ...
    this.http.getApi(`http://localhost:8080/PreSchedule/prettySchedule`).subscribe((res:any)=>{
      ...
      const selectDay = employee.date.find((date: any) =>
        date.applyDate == this.shiftList[index].applyDate
      );
      console.log(selectDay)

      if(!selectDay && prevDays.length >= 4){
        this.dialog.open(Fail, {width: '150px',data: { message: `${this.employeeInfo.employeeId} 該員工已達連續5天上班日`}});
        return;
      }
      ...
      const duplicateShift = selectDay.shiftDetailList.some((item:any)=>
        item.shiftWorkId == this.shiftList[index].shiftWorkId
      )

      const twoShiftWork = !!(selectDay && selectDay.shiftDetailList && selectDay.shiftDetailList.length >= 2);

      if(holiday){
        this.dialog.open(Fail,{width:'150px',data:{message:`${this.employeeInfo.employeeId} 該員工當天休假`}});
        return;
      }

      if(duplicateShift){
        this.dialog.open(Fail,{width:'150px',data:{message:`${this.employeeInfo.employeeId} 該員工當天班別{${this.shiftList[index].shiftWorkId}}重複`}});
        return;
      }

      if(totalHours >= 40){
        this.dialog.open(Fail, {width: '150px',data: { message: `${this.employeeInfo.employeeId} 該員工這周工時已超過`}});
        return;
      }

      if(twoShiftWork){
        this.dialog.open(Fail, {width: '150px',data: { message: `${this.employeeInfo.employeeId} 該員工該天已有兩個班`}});
        return;
      }
  ...
  postData(index:number){
      ...
      this.http.postApi(`http://localhost:8080/PreSchedule/deleteSchedule`,orgData).subscribe((delres:any)=>{
        if(delres.code != 200){
          this.dialog.open(Fail,{width:'150px',data:{message:delres.message}});
          return;
        }

      this.http.postApi(`http://localhost:8080/PreSchedule/addSchedule`,newData).subscribe((updres:any)=>{
        if(updres.code == 200 ){
          this.dialog.open(Success,{width:'150px'});
          this.ngOnInit();
        }else{
          this.dialog.open(Fail,{width:'150px',data:{message:updres.message}});
        }
      });
    });
  }
