...
import { endOfMonth, format, startOfDay, startOfMonth, lastDayOfMonth, addDays, addMonths } from 'date-fns';
...
export class ViewClock {
  ...
  ngOnInit(): void {
    //取得該員工所有打卡天數
    this.http.getApi(`http://localhost:8080/single/history?employeeId=${this.clockInfo.id}&start=${format(this.firstMonth,'yyyy-MM-dd')}&end=${format(lastDayOfMonth(this.firstMonth),'yyyy-MM-dd')}`).subscribe((res:any)=>{
